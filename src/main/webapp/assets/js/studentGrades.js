document.addEventListener('DOMContentLoaded', async () => {
    ...
    try{
        const response = await fetch('api/teacher/all-grades')
        ...
        results.forEach(res =>{
           ...
           row.innerHTML = `
                    ...
                    <td class="score-badge ${badgeClass}">${status}</td>
           `
            tableBody.appendChild(row);
        });
    }catch(error){
        console.error("Failed to load student results ", error);
        tableBody.innerHTML = '<tr><td colspan="5">could not load results</td></tr>'
    }
})